...
class start_hand_eye_calibration(PrimitiveThreadBase):
    ...
    def run(self):
        ...
        self.set_shared_output('started', start)

        self.set_shared_output('hand', self.params['Hand'].value)
        self.set_shared_output('camera', self.params['Camera'].value)
        self.set_shared_output('marker', self.params['Marker'].value)
        self.set_shared_output('base', self.params['Base'].value)
        
        ee_has_camera = self.params['ee_has_camera'].value
        ee_has_camera.setProperty('skiros:Value', self.params['EE has Camera'].value)
        self.set_shared_output('ee_has_camera', ee_has_camera)
